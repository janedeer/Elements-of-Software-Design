...
def groupSumClump(start, nums, target):
    if start >= len(nums):
        ...
        while (start < len(nums) - 1) and nums[start + 1] == nums[start]:
            n += nums[start + 1]
            start += 1
        return groupSumClump(start + 1, nums, target - n) or groupSumClump(start + 1, nums, target)
...
def splitArrayHelper(start, nums, s1, s2):
    if start >= len(nums):
        return s1 == s2
    else:
        return splitArrayHelper(start + 1, nums, s1, s2 + nums[start]) or \
               splitArrayHelper(start + 1, nums, s1 + nums[start], s2)
...
def splitOdd10Helper(start, nums, s1, s2):
    if start >= len(nums):
        return ((s1 % 10 == 0) and (s2 % 2 == 1)) or ((s1 % 1 ==1) and (s2 % 10 == 0))
    else:
        return splitOdd10Helper(start + 1, nums, s1, s2 + nums[start]) or \
               splitOdd10Helper(start + 1, nums, s1 + nums[start], s2)
...
def split53Helper(start, nums, s1, s2):
    if start >= len(nums):
        return s1 == s2
    elif nums[start] % 5 == 0:
        return split53Helper(start + 1, nums, s1 + nums[start], s2)
    elif nums[start] % 3 == 0:
        return split53Helper(start + 1, nums, s1, s2 + nums[start])
    else:
        return split53Helper(start + 1, nums, s1, s2 + nums[start]) or\
               split53Helper(start + 1, nums, s1 + nums[start], s2)
